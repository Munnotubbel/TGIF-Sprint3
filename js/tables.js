...
function tableFunction(members) {
  for (var z = 0; z < members.length; z++) {
    ...
    let numPartyVotes = roundfull(
      (members[z].votes_with_party_pct / 100) *
        ((1 - members[z].missed_votes_pct / 100) * members[z].total_votes)
    );
...
function buildFilter(members) {
  ...
  for (var e = 0; e < stats.statesTable.length; e++) {
    ...
    checkbox.innerHTML =
      '<input type="checkbox" class="checkbox" id="check_' +
      stats.statesTable[e] +
      '" value="' +
      stats.statesTable[e] +
      '">' +
...
function change(puffpuff) {
  ...
  for (var t = 0; t < stats.polPartys.length; t++) {
    // checks if Party-checkboxes  are checked
  ...
  for (var i = 0; i < stats.statesTable.length; i++) {
    // checks if State-checkboxes  are checked
  ...
  var filteredMembers = [];
  for (var zl = 0; zl < puffpuff.length; zl++) {
    ...
    if (filterArr.length === 0 && puffpuff.length === 0) {
      ...
      filterArr.length > 0 &&
      stateArr.length === 0 &&
      filterArr.includes(puffpuff[zl].party)
    ) {
      filteredMembers.push(puffpuff[zl]);
    } else if (
      filterArr.length === 0 &&
      stateArr.length > 0 &&
      stateArr.includes(puffpuff[zl].state)
    ) {
      filteredMembers.push(puffpuff[zl]);
    } else if (
      filterArr.length > 0 &&
